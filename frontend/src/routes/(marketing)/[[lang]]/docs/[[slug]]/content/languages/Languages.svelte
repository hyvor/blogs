...
<p>
	Hyvor Blogs comes with in-built multi-language support. This guide will help you to set up
	languages of your blog correctly. Language settings are located at <strong
		>Settings &rarr; Languages</strong
	>.
</p>
...
<ul>
	<li>
		<strong>Code</strong> - The language code should be a valid
		<a
			href="https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/lang"
			target="_blank"
			rel="nofollow">HTML lang attribute</a
		>
		value. Some valid examples are
		<ul>
			<li><code>en</code></li>
	...
	<li>
		<strong>Name</strong> - The language name is the text that explains the language code. We recommend
		you to write it in the native alphabet. Some themes may use the name to show a message like "This
...
<p>
	When you add a new language, all posts will be translatable to that language. The non-primary
	languages will have index pages, for example, <code>/fr</code> for French. All
	<a href="/docs/routes">routes of your blog</a> will be available in the new language in the same format.
</p>

<h3 id="translate-posts">Step 2: Translate Posts</h3>

<p>
	In the post editor, you will now see an option to switch between languages. When you switch to a
	new language for the first time in a post, a new draft "variant" will be created.
</p>
...
<p>
	Then, you can translate your post content, title, slug, and description to the new language. If
	you use the slug <code>bonjour-monde</code> for the French variant, the URL of the post will be
	<code>/fr/bonjour-monde</code>.
</p>

<h3 id="translate-data">Step 3: Translate Data</h3>

<p>
	All data in your blog can be translated. For example, the blog name, description, navigation
	anchors, etc. Here are the places you can translate data.
</p>
...
<p>
	All official themes are built with multi-language support. You can translate the theme texts in
	YAML files.
</p>

<ol>
	<li>
		Go to the <strong>Theme &rarr; lang</strong> section in the Console. All language files are
		located in the <code>lang</code> folder. <code>en.yaml</code> is the default language file.
	</li>
	<li>
	...
	<li>
		...
		<ul>
			...
			<li>Keep the keys as they are. Only translate the values.</li>
			<li>
...
<p>
	Here are some under the hood works that Hyvor Blogs do to make sure search engine robots
	understand your multi-language pages. You don't need to do anything for these.
</p>
...
<p>
	In addition, HB will add <code>hreflang</code> alternate tags. For example, if you have three
	languages (<code>en</code>, <code>fr</code>, <code>es</code>), the en index page (/) will have
	these tags.
</p>
...
<p>
	For posts, we will add these alternate tags <strong>only if</strong> the translated variants are
