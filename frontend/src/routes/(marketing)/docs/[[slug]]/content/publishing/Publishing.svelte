...
<p>
	All the following requirements should be met in order to publish a theme to our official themes
	list.
</p>
...
<div class="checklist-main">
	<Checkbox>Supports multi-languages (has a language switcher)</Checkbox>
</div>
...
<div class="checklist-main">
	<Checkbox><code>_comments</code> should only be added to posts, not pages.</Checkbox>
</div>
...
<div class="checklist-main">
	<Checkbox><a href="/docs/themes-internationalization">Internationalized</a></Checkbox>
</div>

<div class="checklist-main">
	<Checkbox
		><code>{`<html lang="{{ _lang.code }}" dir="{{ _lang.direction }}"></html>`}</code> is added</Checkbox
	>
</div>

<div class="checklist-main">
	<Checkbox
		>Supports RTL languages. See <a href="/docs/themes-publishing#rtl">RTL Support</a> below.</Checkbox
	>
</div>
...
<div class="checklist-main" id="mult-line">
	<div class="checklist-main-checkbox"><Checkbox></Checkbox></div>
	<div class="checklist-main-text">
		Configuration definitions (<code>config.def.yaml</code>) are added. See
		<a href="/docs/themes-config#config-def">Configuration -> Config Definitions</a>
		page. Use the <a href="/config">config tool</a> to validate <code>config.def.yaml</code>.
	</div>
</div>

<div class="checklist-main">
	<Checkbox>YAML files should use 2 spaces per indentation (not tabs, not 4 spaces).</Checkbox>
</div>
...
<div class="checklist-main">
	<Checkbox>Content styles are added. See Content styles section below.</Checkbox>
</div>

<div class="checklist-main" id="mult-line">
	<div class="checklist-main-checkbox"><Checkbox></Checkbox></div>
	<div class="checklist-main-text">
		All assets (JS, fonts, etc.) should be added in the <code>assets</code> folder. Do not load assets
		from external sources like Google Fonts.
	</div>
</div>
...
<p>
	As explained in the <a href="/docs/themes-config">configurations</a> page, the following configurations
	are required when publishing your theme.
</p>
<CodeBlock
	language="yaml"
	code={`
...
<ul>
	...
	<li>
		Use direction-aware CSS properties when adding horizontal padding, margin, and left/right
		borders
	</li>
</ul>
...
<p>
	These heading anchor should be <b>styled differently</b> from other links. For example, you can
	add a <code>#</code> or an SVG image (via <code>background-image</code>) before the anchor text.
</p>
<CodeBlock
	language="css"
	code={`
...
<ul>
	...
	<li>
		Add left padding when line numbers are enabled (check for <code>.has-line-numbers</code>).
	</li>
</ul>

<p>
	And, line numbers should be absolutely positioned. Also, add left padding when line numbers are
	enabled (check for <code>.has-line-numbers</code>).
</p>
...
<p>
	Margins must be handled carefully for paragraphs inside lists, tables, and blockquotes. The
	following works well in most cases.
</p>
...
<p>
	The first version of the theme should be <code>1.0.0</code>. After that you can increment the
	version number according to the changes you make. For a patch (ex: bug fix), you can use
	<code>1.0.1</code>, <code>1.0.2</code>, etc. For a minor change, you can use <code>1.1.0</code>,
	<code>1.2.0</code>, etc. Unlike other software, themes do not have significant changes breaking
	changes. Therefore, we do not think you will ever need a major version change.
</p>
...
<p>
	Once everything is completed, send us a pull request to <a
		href="https://github.com/hyvor/hyvor-blogs-themes">hyvor-blogs-themes</a
	> repository. When the PR is merged, the themes list will automatically update with your new theme.
</p>
