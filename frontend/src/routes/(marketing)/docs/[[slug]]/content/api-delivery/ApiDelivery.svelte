...
<p>
    Delivery API tells you information about how to "serve a request". This API is the backbone of <a href="/docs/subdirectory">hosting a blog on a subdirectory</a>.
</p>

<p>
    <strong>API Endpoint</strong>: <code>https://blogs.hyvor.com/api/delivery/v0/{`{subdomain}`}</code>
</p>
...
<ul>
    ...
    <li>
        Set <code>api_key</code> query parameter to a valid Delivery API key. You can create one at <strong>Settings &rarr; API Keys</strong>.
    </li>
    <li>
        Set <code>path</code> query parameter to the path of the blog you want to get information about. For example, if you want to get information about <code>https://myblog.com/hello-world</code>, set <code>path</code> to <code>/hello-world/</code>.
    </li>
</ul>
...
<Table columns="1fr 1fr 2fr">
    ...
    <TableRow>
        ...
        <div>
            Whether the response object should be cached in proxy/intermediary servers. <code>false</code> for post preview routes.
        </div>
    </TableRow>
    <TableRow>
        ...
        <div><code>integer</code></div>
        <div>
            HTTP status code of the response. Can be <code>200</code>, <code>301</code>, <code>302</code>, or <code>404</code>.
        </div>
    </TableRow>
</Table>
