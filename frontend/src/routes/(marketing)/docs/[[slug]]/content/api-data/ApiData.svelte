...
<ul>
	...
	<li>
		For example, if your blog is at <code>https://example.hyvor.com</code>, the base path is
		<code>https://blogs.hyvor.com/api/data/v0/example</code>
	</li>
	<ul>
...
<p>
	For single-object endpoints, the response is an object. For example, <code>/post</code> endpoint
	returns a <code>Post</code> object (See below for object definitions).
</p>
...
<p>
	The <code>/blog</code> endpoint only takes <code>language</code> and <code>keys</code> as an input.
</p>

<h3 id="multi-object">Multi-Object endpoints</h3>

<p>
	<code>/posts</code>, <code>/posts/search</code>, <code>/tags</code>, and <code>/authors</code>
</p>
...
<p>
	The <code>/posts/search</code> endpoint has a required <code>search</code> param in addition to the
	above params.
</p>
...
<p>
	The <code>/tags</code> endpoint has an optional <code>visibility</code> param to filter tags by
	visibility. Note that <a href="/docs/tags#private">private tags</a>
	are not meant to be shown in the blog publicly. They should only be used for internal purposes (ex:
	show/hide a widget in the blog if the tag is present in the post).
</p>
...
<p>
	If your blog has <a href="/docs/languages">multiple languages</a>, you can set the
	<code>language</code>
	param to a language code (ex: <code>en</code>, <code>fr</code>) of a language in your blog. If
	this param is not provided, the primary language of the blog is used. That language will be used
	to localize strings in posts, authors, tags, and the blog.
</p>
...
<Callout type="info">
	<p>
		...
		you call the <code>/posts</code> endpoint with language the language code <code>fr</code>,
		...
		<code>fr</code>
		variant or not. Missing translations will be filled with primary language strings. The reason is
		...
		fallbacking author/tags data is fine in most cases. <br />
		<Divider />
...
<p>
	The <code>limit</code> param can be used to limit the number of records returned in multi-object
	endpoints. The default is <code>25</code>. Max is <code>250</code>.
</p>

<h4 id="page">3. <code>page</code> param</h4>
<p>
	The <code>page</code> param can be used to paginate results. This works in combination with the
	limit param. The default value is <code>1</code>.
</p>
<CodeBlock
	code={`
...
<p>
	Example: <code
		>{`(published_at > 1639665890 & published_at < 1639695890) | is_featured=true`}</code
	>
</p>
<p>
...
<ul>
	...
	<ul>
		<li>
			Strings without quotes should match <code>[a-zA-Z_][a-zA-Z0-9_-]+</code> and cannot be
...
<p>
	Please see the <a href="https://github.com/hyvor/laravel-filterq#filterq-expressions"
		>FilterQ Expressions</a
...
<p>
	For example: In <code>/posts</code> endpoint, you may use <code>{`published_at>'-7 days'`}</code> to
...
<ul>
	...
	<li>
		<code>is_featured DESC</code>, <code>published_at DESC</code> - featured posts first, then
		ordered by publish time in descending order. <code>DESC</code> is optional.
		<code>is_featured</code>, <code>published_at</code> is identical with the former.
	</li>
</ul>
...
<p>
	Let's say you only want to get the post ID and tag ID of the posts. Use <code
		>keys=id,tags.id</code
...
<p>
	A pagination object is included in all multi-object endpoints (<code>/posts</code>,
	<code>/authors</code>, <code>/tags</code>).
</p>
...
<p>
	5xx errors means something is wrong on our side. Check our <a href="https://status.hyvor.com/"
		>status page</a
	>
	for any downtimes. If the issue persists, <a href="/docs/support">contact us</a>.
</p>
...
<ul>
	...
	<li>
		To get multiple pages, call the <code>/posts</code> endpoint with <code>?pages=true</code> param.
	</li>
