...
<h1>Routes</h1> 
    <p>Routes are used to configure how specific requests are handled. A route matches the request path and determines what output to send back to the user. For example, if the request path is <code>/tag/my-tag</code>, route determines that it should render <code>tag.twig</code> and send response back to the user.</p>
...
<h2 id="defaults">Default Routes</h2>
    <p>A new blog will have these 5 default routes.</p>
        ...
        <Callout type="info">
            <p>Posts Filter is a <a href="https://github.com/hyvor/laravel-filterq">FilterQ expression</a> to filter posts. These filtered posts will be sent to the template as the <code>_posts</code> variable. It is only used in listing pages like index, tag, author. Matched params (<code>{`{slug}`}</code>) can be used in this expression.</p>
        </Callout>
        ...
        <Table columns="2fr 2fr" hover>
            ...
            <TableRow>
                <div><code>{`/sitemap.xml`}</code>, <code>{`/sitemap-pages.xml`}</code>, <code>{`/sitemap-posts-x.xml`}</code></div>
...
<h2 id="permalinks">Customizing Post/Page Permalinks</h2>
        ...
        <p>You can change the match value of <code>post</code> and <code>page</code> routes to change post/page permalinks. By default, it looks like <code>{`/{slug}`}</code>. You may change it to a different structure which may have date, tag, and/or author name. Here are some examples.</p>
        ...
        <p>Note that <code>{`{slug}`}</code> is always required. The following placeholders are supported.</p>
        ...
        <Callout type="info">
            <p>Please note that if you use <code>{`{tag}`}</code> (or <code>{`{author}`}</code>) in the post/page URL, all post <b>should have</b> at least one tag (or author). Otherwise, the post/page URL will show 404 error.</p>
        </Callout>
    ...
    <h2 id="customizing-other">Customizing Other Default Routes</h2>
        <p>Similar to <code>post</code> and <code>page</code>, <code>routes</code>, other default routes (<code>index</code>, <code>tag</code>, <code>author</code>, and <code>search</code>) routes are customizable to some extent. Here are some ideas:</p>
            <ul>
            ...
            <Callout type="info">
                <p>Please note that you (or theme developers) can also add custom routes by adding <code>{`route-{route}`}</code>.twig files to template files. See <a href="/docs/themes-templates#custom-routes">here</a> from more details.</p>
            </Callout>

    <h2 id="custom">Custom Routes</h2>
        <p>In addition to the default 6 routes, you can add your own routes. Some examples use cases are:</p>
            <ul>
    ...
    <h2>Suffixes and Prefixes</h2>
            <p>These suffixes are supported:</p>
                ...
                <p>Matches can be prefixed with a language code. See languages for setting up multiple <a href="/docs/languages">languages</a> on your blog.</p>
            
    <h2 id="website">Making a Website</h2>
            <p>We usually call a website "a blog" when it has posts and the home page lists all of them. That is the default behavior of Hyvor Blogs. Even out of its purpose, you can use Hyvor Blogs to create a general website. For example, you can create a landing page for the homepage, and have your blog in the <code>/blog</code> subdirectory.</p>

    <h2 id="collections">Multiple Post Collections</h2>
    <p>By default, your blog has one post collection, and all posts will be listed the index page. What if you want to have to separate collections, for example, blog posts and podcast episodes in the blog? You can use routes and <a href="/docs/tags">tags</a> to achieve this.</p>

    <ul>
        <li>We add a <code>podcast</code> tag to the all podcast posts.</li>
        <li>We can customize the <code>index</code> (<code>/</code>) route and its template (<code>index.twig</code>) to show an overview of blog posts and podcast episodes. To do this, you will need to update <code>index.twig</code> and use our <a href="/docs/api-data">Data API</a> to fetch posts separately.</li>
        <li>We add two new routes with a new template:</li>
            <ul>
                <li><code>/blog</code> -> to list blog posts (using the filter <code>{`tag.slug != podcast`}</code>)</li>
                <li><code>/podcast</code> -> to list podcast episodes (using the filter <code>{`tag.slug = podcast`}</code>)</li>
            </ul>
    </ul>
