...
<h1>Styling Themes</h1>
    ...
    <p>This folder contains SCSS files. <code>index.scss is required.</p>
    <p>While developing and working with other blogging platforms/CMSs, we understood that customizing a theme becomes really hard when the theme developer puts all CSS in a single file. Therefore, we decided that we want to support "chunk-css" files to make it easy to edit for the blogger. And, we use <a href="https://sass-lang.com/" rel="nofollow">SCSS</a> instead of CSS to make the theme developer's life easier. All CSS is valid SCSS. So, if you haven't use SCSS earlier, just use CSS. SCSS just have some cool features like nesting rules.</p>
    ...
    <Callout type="info">
        <p>We strongly encourage you to write CSS from scratch without using any libraries like Bootstrap. A blog theme is very simple and it is totally possible to manage everything on your own without depending on third-party libraries. If you really want to use a library, add it to assets instead of styles.</p>
    </Callout>
        ...
        <h2 id="advanced-nodes">Advanced Nodes</h2>
                ...
                <p><a href="/docs/writing">Writing</a> page describes all supported nodes. We try to use the most basic HTML elements to represent each node. However, there are some advanced components that require some attention when writing styles.</p>
        ...
        <h2 id="light-dark">Light/Dark Modes</h2>
            ...
            <p>It would be troublesome for you to write logic to consider all these options and find out what theme to show to the user. Therefore, we make developing light/dark easy by adding a class name to the <code>{`<html></html>`}</code> element. You can decide colors based on that class. We recommend you to use <a href="https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties" rel="nofollow">CSS Variables</a> to define colors in <code>colors.scss</code> file.</p>
            ...
            <p>That is all you have to do to support light and dark modes. We take care of showing the correct theme to the user.</p>

            <Callout type="info">
                 <p>Under the hood, determining the color is handled by a small Javascript code injected into the blog in the <code>_head</code> <a href="/docs/themes-templates#placeholders">placeholder</a>.</p>
            </Callout>

        <h2 id="mode-toggler">Light/Dark Mode Toggler</h2>

            <p>If you support both light and dark themes, you will mostly likely have a button that allows users to toggle between color modes. In most other platforms, you have to write logic to do this manually and save preferences in local storage - but not in Hyvor Blogs!</p>
            <p>We mentioned above that we add a small Javascript code to help you with determining light/dark modes. It also exposes a simple API to help you with toggling modes.</p>
            ...
            <p>You can use a SCSS like this to show buttons on the based on the theme. This will show the light mode icon in dark mode, and dark mode button in light mode.</p>
        ...
        <h2 id="mode-toggler-os">Light + Dark + OS Preference Toggler</h2>
            <p>Some may also want to add OS preference option to the toggler. In that case, the HTML will be similar but we have to use <code>mode-preference-*</code> classes to detect the user's preference.</p>

                <ul>
                    <li><code>mode-light</code> and <code>mode-dark</code> classes are added to the <code>{`<html>`}</code>, and they represent current <b>color mode</b>.</li>
                    <li><code>mode-preference-light</code>, <code>mode-preference-dark</code>, <code>mode-preference-os</code> are added to the <code>{`<html>`}</code> and they represent the current <b>color mode preference</b>.</li>
                </ul>
                ...
                <Callout type="info">
                    <p>Please note that these examples are just here to explain you how it works. Feel free to design more creative color mode togglers ;)</p>
                </Callout>
