...
<p>
	...
	Javascript, you can easily build your own theme from scratch. This page is an overview to help
	you get started. All official themes are in the <a
...
<ul>
	...
	<li>
		<b>Blogger</b> - The person who owns the blog. They can install the theme you create and edit
		it through the console.
	</li>
	<li>
		<b>Subdomain</b> - Subdomain part of <code>{`{subdomain}`}</code>.hyvorblogs.io which is
		given to the blogger.
	</li>
	<li>
		<b>Route</b> - Routes of the blog that determines how to render a page or what output to
		return for a specific URL path. See <a href="/docs/routes">routes</a>
	</li>
	<li><b>HB</b> - Hyvor Blogs</li>
	<li>
		<b>Rendering</b> - Combining a theme (template) with blog data and returning HTML output. See
		the below image.
	</li>
</ul>
...
<ul>
	...
	<li>
		Styling supports <a href="https://sass-lang.com/" rel="nofollow">SCSS</a>, but you may just
		use CSS
	</li>
	<li>
...
<p>
	...
	Therefore, it is important to keep Flashload in mind while designing themes. Please take a
	minute and read the
	<a href="https://github.com/hyvor/flashload#readme">Flashload documentation</a> to get the idea of
	how it works.
</p>
<p>
	Why Flashload? Browser reloads are slow. They load the same CSS/JS resources multiple times
	making page rendering slower. Flashload starts loading other pages even before the user clicks
	the link. It makes navigation smoother. It simply turns the blog into a <b
		>Single Page Application (SPA)</b
	>!
</p>
...
<ul>
	...
	<li>
		We don't have any cached output for this path. We fetch data from our database, combine it
		with the template, and generate the HTML output, and send the response back to the user.
		Behind the scenes, we save the generated HTML output in our cache.
	</li>
	<li>
		When someone else requests the same path, the HTML output is directly sent from the cache.
		It does not go through the rendering process.
	</li>
</ul>

<p>
	When using a cache, clearing cache is the most important thing. We have to make sure outdated
	content is not delivered when something changes. Here are the events that we clear cache for
	each scope.
</p>
...
<ul>
	<li>
		<code>/search</code> and <code>{`/p/{hash}`}</code> (preview pages) routes are always dynamic,
		never cached.
	</li>
</ul>
...
<Callout type="info">
	<p>
		⚠️ Caching makes the blog super fast. However, it puts some limitations to theme
		development. You can't render dynamic data like "current date" using Twig. Due to cache,
		users may see an old date. If absolutely required, you have to use Javascript to render
		dynamic content inside user's browser. However, displaying the "publish date" of a post
		works fine because we clear cache whenever the post is updated. Also, displaying the current
		year will work, because we will make sure to clear the cache on the 1st of January.
	</p>
</Callout>
...
<ul>
	<li>
		First, you need a DEV blog. Create one at <a href="/console/new/dev">/console/new/dev</a>.
		DEV blogs are similar to normal blogs in Hyvor Blogs, however they are free, and can only be
		used for theme development.
	</li>
	<li>
		You will get a subdomain in the <code>dev-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx</code> format.
...
<ul>
	<li>
		Create a new directory in your computer, which will contain all theme files and
		configurations.
	</li>
</ul>
...
<Callout type="info">
	<p>
		<b>How it works:</b> The <code>hyvor-blogs-cli</code> command runs a process that watches your
		local file changes and syncs it with our production environment. So, whenever you add, edit, or
		delete a file within your theme folder, it will be synced with the theme files in your DEV blog.
	</p>
	...
	<p>
		<b>Security Notice 2:</b> Do not share your DEV subdomain publicly. It will allow other
		users to change theme files in your DEV blog. If you are using GIT for versioning, make sure
		to add
		<code>.env</code>
...
<ul>
	...
	<li>
		<b>assets</b>: You may add SVGs, PNGs, font files, or Javascript files here. All files in
		this directory are publicly accessible via the /assets <code>{`/{file_name}`}</code> route. Any
