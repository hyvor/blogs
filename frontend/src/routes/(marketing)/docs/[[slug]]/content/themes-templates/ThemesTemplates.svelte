...
<h1>Templates</h1>
    ...
    <p>When a blog gets a request, first, we match its path to a Route (let's assume the route <code>post</code> for <code>/hello-world</code>). Then, we fetch required data from our database, then we call the Twig template file defined in that route (<code>post.twig</code>).</p>

    <p>Inside this file, you can include other files or even use <a href="https://twig.symfony.com/doc/3.x/templates.html#template-inheritance" rel=nofollow>inheritance</a>. You can even call our <a href="/docs/api-data">Data API</a> to fetch more data (More on that below)!</p>

        <h2 id="twig">Twig</h2>
            <p>We use <a href="https://twig.symfony.com/doc/3.x/" rel="nofollow">Twig 3.0</a> for templating. It is a powerful language with a plenty of in-built tags, filters, and functions. Twig also has nice, easy-to-follow documentation, which was one reason we chose Twig over other template languages. If you haven't used it ever, go through the <a href="https://twig.symfony.com/doc/3.x/templates.html" rel="nofollow">Twig for Template Designers</a> page, and you will get an idea of how it works. Basically, it's HTML with superpowers.</p>
                ...
                <h3>Templates</h3>
                    ...
                    <Table columns="1fr 3fr 2fr" hover>
                        ...
                        <TableRow>
                            <div>Partial</div>
                            <div>These templates are not rendered directly but included in main template files. They start with an underscore (<code>_</code>)</div>
                            <div><code>_footer.twig</code></div>
                        </TableRow>

                        <TableRow>
                            <div>Route</div>
                            <div>These templates are used to define custom routes for a blog. The file name starts with <code>route-</code>. See <a href="/docs/themes-templates#custom-routes">custom routes</a> below</div>
                            <div><code>route-authors.twig</code></div>
                        </TableRow>

                        <TableRow>
                            <div>Component</div>
                            <div>These templates are used to define new HTML structures for complex components like link previews. See <a href="/docs/themes-templates#layouts">Embed: Link</a>.</div>
    ...
    <h2 id="variables">Theme Variables</h2>
            ...
            <p>When rendering the twig templates, we send data into your template file as objects. You will use this data to generate a beautiful UI.</p>
            <p>There are 4 main objects in HB: <code>Blog</code> , <code>Post</code> , <code>Tag</code> , and <code>Author</code>. These objects are explained in the <a href="/docs/api-data">Data API</a> page.</p>
            ...
            <Table columns="1fr 1fr 3fr" hover>
                ...
                <TableRow>
                    ...
                    <div>(all)</div>
                    <div>A Language Object for the <b>current</b> language. Should also be placed in <code>{`<html lang="{{ _lang.code }}">`}</code></div>   
                </TableRow>
                ...
                <TableRow>
                    ...
                    <div>(all)</div>
                    <div>An array of Post objects, filtered by the <a href="/docs/routes">route</a>'s filter value</div>
                </TableRow>
            ...
            <p>Each Route gets different variables. We prefix each variable with <code>_</code> so that it won't conflict with the variables you define inside the theme files (Obviously, you shouldn't prefix <code>_</code> your variables inside the Twig template)</p>
    ...
    <h2 id="placeholders">Placeholders</h2>
        ...
        <p>You are required to put some placeholders in your theme to make a few things work.</p>
            ...
            <Table columns="1fr 1fr 4fr" hover>
                ...
                <TableRow>
                    ...
                    <div>(all)</div>
                    <div>place before <code>{`</head>`}</code>. We automatically add SEO tags, styles.css link, and code_head set by the blogger</div>
                </TableRow>
                ...
                <TableRow>
                    ...
                    <div>post and page</div>
                    <div>to render the comment count of that page. For example, some themes have comment count at the top with a link to the comments section to encourage more comments. Only works when Hyvor Talk is connected</div>
                </TableRow>
        ...
        <p>Sending all placeholders (except <code> _lang</code>) through the <code>template</code> filter is absolutely required to make them render as templates.</p>
        ...
        <Callout type="info">
            <p><code>{`{{ _head | template }}`}</code> is equal to <code>{`{{ include(template_from_string(_head)) }}`}</code> in Twig. We defined the custom <code>template</code> filter to make it easier for you to write it, as it is used frequently in HB templates.</p>
        </Callout>
                    ...
                    <ul>
                        ...
                        <li>These icon libraries are supported</li>
                            ...
                            <ul>
                                ...
                                <Callout type="info">
                                    <p>Under the hood, we use the <a href="https://github.com/hyvor/php-svg-icons">php-svg-icons</a> open-source library. If you need to add more icon libraries, please send a PR there.</p>
                                </Callout>
                            </ul>
                    </ul>
    ...
    <h2 id="fetch-data">Fetching Data</h2>
        <p>Use the <code>data</code> function to fetch data from our <a href="/docs/api-data">Data API</a>.</p>
        <CodeBlock code={`
        <!-- Fetch data -->
        ...
        <p>Use the <code>endpoint</code> named argument to set the API endpoint. You can set all other parameters by just sending them as named arguments to the <code>data</code> Twig function (Ex: <code>sort="published_at DESC"</code>).</p>

    <h2 id="custom-routes">Custom Routes</h2>
        <p>There are two ways to add custom routes:</p>
        <ul>
            <li>The blogger can add custom routes from the console (<a href="/docs/routes#custom">See docs</a>).</li>
            <li>Theme developers can define custom routes by adding files named <code>{`route-{route}.twig`}</code> to the <code>templates</code> folder.</li>
        </ul>

        <p>The first option is more robust, and it providers easier way to automatically set input variables <coe>_posts</coe> (by filtering), <code>_tag</code>, <code>_author</code>, etc so you can access them without calling the Data API. But, as a theme developer, you will need to use the second option.</p>
        <p>For example, let's say you decide that your theme want a page to list all authors of the blog. You can add a <code>{`route-authors.twig`}</code> to the <code>templates</code> folder. If the blog gets a request to <coe>/authors</coe>, this template will be rendered automatically.</p>