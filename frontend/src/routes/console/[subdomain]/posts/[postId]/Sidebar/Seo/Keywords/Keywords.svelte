<script lang="ts">
    ...
    function addPrimaryKeyword(k: string) {
        ...
        if ($postVariantStore.seo_secondary_keywords.find(
            keyword => keyword.toLowerCase() === k
        )) {
    ...
    function addSecondaryKeyword(keyword: string | null, newKeyword: string) {
        ...
        if (keyword !== null) {
            updateSecondaryKeywords($postVariantStore.seo_secondary_keywords.map(
                k => k === keyword ? newKeyword : k
            ));
        } else {
            updateSecondaryKeywords([
                ...$postVariantStore.seo_secondary_keywords,
                newKeyword
            ]);
        }
    ...
    function handleSecondaryRemove(keyword: string) {
        updateSecondaryKeywords($postVariantStore.seo_secondary_keywords.filter(
            k => k !== keyword
        ));
    }
