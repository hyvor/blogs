...
export const selectedThemeFileStore = derived(
  ...
  ([$themeFiles, $selectedThemeFileId]) => {
    return $themeFiles.find(
      (themeFile) => themeFile.id === $selectedThemeFileId,
    );
  },
);
...
export function setThemeFiles(files: ThemeFile[]) {
  ...
  function getFocusFileId(files: ThemeFile[]) {
    const configYaml = files.find(
      (file) => file.folder === null && file.name === "config.yaml",
    );
...
export function updateThemeFileStore(
  id: number,
  data: Partial<ThemeFile>,
  original = false,
) {
