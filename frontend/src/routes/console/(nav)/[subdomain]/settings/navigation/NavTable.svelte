<script lang="ts">
	...
	async function handleDelete(id: number) {
		if (
			await confirm({
				title: i18n.t('console.settings.navigation.deleteTitle'),
				content: i18n.t('console.settings.navigation.deleteContent'),
				confirmText: i18n.t('console.settings.navigation.deleteConfirm'),
				danger: true
			})
		) {
...
<div class="wrap">
	<Table columns="70px 1fr 1fr 70px">
		<TableRow head>
			...
			<div>{i18n.t('console.settings.navigation.url')}</div>
			<div></div>
		</TableRow>
		...
		>
			{#each items as item (item.id)}
				<div class="global-navigation-item-row" animate:flip={{ duration: flipDurationMs }}>
					...
					<div>{item.variants[0]?.name || i18n.t('console.common.unnamed')}</div>
					<div>{item.url}</div>
					<div>
