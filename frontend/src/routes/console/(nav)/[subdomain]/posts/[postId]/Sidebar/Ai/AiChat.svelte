<script lang="ts">
	...
	const automaticPrompts: AutomaticPrompt[] = [
...

		{
			...
			prompt: (options) =>
				addKeywordPrompt(
					`Write a list of frequently asked questions about "${options.title}" and provide answers to them considering SERP and rich result guidelines.`,
					options
				)
		},

		{
			...
			prompt: (options) =>
				addKeywordPrompt(`Write an SEO content brief for ${options.title}.`, options)
		},
