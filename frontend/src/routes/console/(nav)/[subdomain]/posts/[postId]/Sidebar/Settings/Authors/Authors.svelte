<script lang="ts">
	...
	function handleRemoveAuthor(authorId: number) {
		...
		if ($postVariantStore.status !== 'published') saveAuthors();
	}
	...
	let hasAuthorsChanged = $derived(
		hasIdArrayChanged($postStore.authors, $postOriginalStore.authors)
	);
</script>
		...
		<div class="authors">
			...
			<div class="right">
				<Dropdown position="bottom" align="end" width={300} bind:show={dropdownOpen}>
					...
					{#snippet content()}
						<AuthorsSearch on:select={handleAddAuthor} />
					{/snippet}
				</Dropdown>
			</div>
		</div>
