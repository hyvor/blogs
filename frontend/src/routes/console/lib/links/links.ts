...
import {
  getDocFromContent,
  positionSelectionInMiddleOfScreen,
...
export function getLinksFromContent(
  content: string | null,
  baseUrl: string,
): Link[] {
...
export function getLinkType(href: string, baseUrl: string): LinkType {
  ...
  if (hrefFullUrl.protocol !== "https:" && hrefFullUrl.protocol !== "http:")
    return "other";

  const { domain: blogDomain, subdomain: blogSubdomain } = parse(baseUrl);
  const { domain: hrefDomain, subdomain: hrefSubdomain } = parse(
    hrefFullUrl.toString(),
  );
