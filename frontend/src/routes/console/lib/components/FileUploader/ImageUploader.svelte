<script lang="ts">
	import type { SelectedImage as SelectedImageType } from './image-uploader';
...
<div class="image-uploader">
	<Modal bind:show size="large" closeOnEscape={false} closeOnOutsideClick={false}>
		...
		<div class="body" style:position={selectedImage ? 'relative' : undefined}>
			{#if tab === 'upload'}
			...
			{#if selectedImage}
				<SelectedImage image={selectedImage} on:select={handleFinish} />
			{/if}
		</div>
	</Modal>
</div>
