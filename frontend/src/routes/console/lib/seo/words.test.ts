...
test("get words", () => {
  ...
  expect(getWords("随着数字时代的到来")).toStrictEqual([
    "随着",
    "数字",
    "时代",
    "的",
  ...
  expect(getWords("こんにちは こんにちは")).toStrictEqual([
    "こんにちは",
    "こんにちは",
  ]);
  expect(getWords("こんにちは、こんにちは")).toStrictEqual([
  ...
  expect(getWords("안녕하세요 안녕하세요")).toStrictEqual([
    "안녕하세요",
    "안녕하세요",
  ]);
  expect(getWords("안녕하세요, 안녕하세요")).toStrictEqual([
...
describe("occurrences of keyword", () => {
  ...
  test("occurrences of keyword in content", () => {
    ...
    expect(
      getOccurrencesOfKeywordInContent("Word", "Word not Word and word"),
    ).toBe(3);
    ...
    expect(
      getOccurrencesOfKeywordInContent(
        "blogging platform",
        "Hyvor Blogs is a blogging platform",
      ),
    ).toBe(1);
    ...
    expect(
      getOccurrencesOfKeywordInContent("你好", "你好 随着数字时代的到来"),
    ).toBe(1);

    // japanese
    expect(getOccurrencesOfKeywordInContent("こんにちは", "こんにちは")).toBe(
      1,
    );
    expect(
      getOccurrencesOfKeywordInContent("こんにちは", "こんにちは こんにちは"),
    ).toBe(2);

    // bug
    expect(
      getOccurrencesOfKeywordInContent("blogging platform", "blogging"),
    ).toBe(0);
  });
