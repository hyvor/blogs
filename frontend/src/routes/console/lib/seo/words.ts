...
export function getWords(str: string, languageCode?: string | undefined) {
  ...
  if (typeof Intl.Segmenter === "function") {
    const segmenter = new Intl.Segmenter(getIntlLanguageCode(languageCode), {
      granularity: "word",
    });
...
export function getOccurrencesOfKeywordInContent(
  keyword: string,
  content: string,
  languageCode?: string | undefined,
) {
  ...
  contentWords.forEach((contentWord, i) => {
    if (contentWord.toLowerCase() !== keywordWords[0]!.toLowerCase()) return;
...
function getOccurrencesOfKeywordInContentWithoutIntl(
  keyword: string,
  content: string,
) {
