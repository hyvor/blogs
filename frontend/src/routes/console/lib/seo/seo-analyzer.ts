...
import {
  getOccurrencesOfKeywordInContent,
  getWords,
  getWordsCount,
} from "./words";
...
export class Test {
  ...
  protected keywordInString(keyword: string, str: string): boolean {
    ...
    return (
      getOccurrencesOfKeywordInContent(keyword, str, this.input.languageCode) >
      0
    );
  }
}
...
export class PrimaryKeywordInTitleTest extends Test {
  public run() {
    ...
    if (!this.input.primaryKeyword) {
      result.ignore = true;
      result.message =
        "Ignored primary keyword in title test. Add primary keyword";
      return result;
    }
    ...
    if (title.startsWith(primaryKeyword)) {
      ...
      result.score = 75;
      result.message =
        "Primary keyword found in the title, but not at the beginning";
    } else if (title.includes(primaryKeyword)) {
      result.score = 49;
      result.message =
        "Primary keyword found in the title, but not within first 50 characters";
    }
...
export class PrimaryKeywordInDescriptionTest extends Test {
  public run() {
    let result = this.defaultResult(
      "Primary keyword not found in the description",
    );

    if (!this.input.primaryKeyword) {
      result.ignore = true;
      result.message =
        "Ignored primary keyword in description test. Add primary keyword";
      return result;
    }
...
export class PrimaryKeywordInSlugTest extends Test {
  public run() {
    ...
    if (!this.input.primaryKeyword) {
      result.ignore = true;
      result.message =
        "Ignored primary keyword in slug test. Add primary keyword";
      return result;
    }
...
export class PrimaryKeywordInBeginningOfContentTest extends Test {
  public run() {
    let result = this.defaultResult(
      "Primary keyword not found in the beginning of the content",
    );

    if (!this.input.primaryKeyword) {
      result.ignore = true;
      result.message =
        "Ignored primary keyword in beginning test. Add primary keyword";
      return result;
    }
  ...
  public run() {
    ...
    result.message =
      result.score === 100
        ...
        : "Some keywords are missing in the content: " +
...
export class AllKeywordsInSubHeadingsTest extends Test {
  public run() {
    ...
    for (let subHeading of subHeadings) {
      ...
      for (let keyword of keywords) {
        if (
          this.keywordInString(keyword, text) &&
    ...
    if (notFound.length === 0) {
      ...
      result.message =
        "Some keywords not found in subheadings: " + notFound.join(", ");
    }
...
export class AllKeywordsInImgAltTest extends Test {
  public run() {
    const result = this.defaultResult(
      "No keywords found in image alt attributes",
    );
    const keywords = this.allKeywords();
    ...
    for (let image of images) {
      ...
      for (let keyword of keywords) {
        if (
          this.keywordInString(keyword, alt) &&
          !foundKeywords.includes(keyword)
        ) {
          foundKeywords.push(keyword);
        }
    ...
    if (notFound.length === 0) {
      ...
      result.message =
        "Some keywords not found in image alt attributes: " +
        notFound.join(", ");
    }
...
export class ExternalLinksTest extends Test {
  public run() {
    ...
    const externalLinksCount = this.links().filter(
