...
class WordPressParser extends MediaAwareParserAbstract
{
    ...
    /**
     * Caches the slugs of imported posts, used to ignore duplicates
     * @var string[]
     */
    ...
    public function __construct(
        ...
        $this->blogUrl = $this->permalinkService->getBlogUrl($this->blog);
    }
    ...
    private function parsePost(string $postXml): void
    {
        try {
            ...
            $this->log->info('Skipping post due to error: ' . $e->getMessage());
            return;
        }
        ...
        $pubDate = new \DateTimeImmutable((string)$this->element($post, 'pubDate'));
    ...
    private function parseAttachment(string $xml): void
    {
        try {
            ...
            $this->log->info('Skipping attachment due to error: ' . $e->getMessage());
            return;
        }
