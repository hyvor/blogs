...
class PermalinkRepository
{
    ...
    /**
     * Gets permalink of a post/page by post, blog and language
    ...
    public static function getPostVariantPermalink(
        ...
        $path = preg_replace_callback($regex, function ($matches) use ($variant) {
            if ($variant->post->published_at) {
                return $variant->post->published_at->format(self::DATE_FORMATTERS[$matches[1]]);
            }
        }, $path);
    ...
    public static function getAuthorPermalink(
        User $author,
        Blog $blog,
        Language $language,
        bool $onlyPath = false
